...
@Injectable()
export class CollectionPositionService {
  generateBetweenPosition(prev: string, next: string): string {
  ...
  private _findLeftmostNonMatchCharacter(prev: string, next: string): number {
    let p: number, n: number, pos: number;
    for (pos = 0; p == n; pos++) {
      // find leftmost non-matching character
      p = pos < prev.length ? prev.charCodeAt(pos) : 96;
      n = pos < next.length ? next.charCodeAt(pos) : 123;
